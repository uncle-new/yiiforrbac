        ...
        if ($model->load(Yii::$app->request->post())&&$model->validate()){
            ...
            if($model->save(false)) {
                foreach ($post['role'] as $v) {
                    /*获取当前所要授权的角色*/
                    $authorRole = $auth->getRole($v);
                    $auth->assign($authorRole, $model->attributes['id']);
                }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())&&$model->validate()){
            ...
            return $this->render('update', [
                'model' => $model,
                'roles' => \backend\models\Rbac::getRoles(),    /*获取已有角色列表*/
                'role' => \backend\models\Rbac::getUserRole($auth->getRolesByUser($id)),    /*获取用户已授权的角色列表*/
            ]);
        }
    }
